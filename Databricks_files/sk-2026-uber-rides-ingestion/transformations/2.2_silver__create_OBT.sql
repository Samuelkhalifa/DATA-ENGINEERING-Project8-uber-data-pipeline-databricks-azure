...
FROM
            ...
            LEFT JOIN uber.bronze.map_vehicle_makes AS map_vehicle_makes ON stg_rides.vehicle_make_id = map_vehicle_makes.vehicle_make_id 
        
    
        
            LEFT JOIN uber.bronze.map_vehicle_types AS map_vehicle_types ON stg_rides.vehicle_type_id = map_vehicle_types.vehicle_type_id 
        
    
        
            LEFT JOIN uber.bronze.map_ride_statuses AS map_ride_statuses ON stg_rides.ride_status_id = map_ride_statuses.ride_status_id 
        
    
        
            LEFT JOIN uber.bronze.map_payment_methods AS map_payment_methods ON stg_rides.payment_method_id = map_payment_methods.payment_method_id 
        
    
        
            LEFT JOIN uber.bronze.map_cities AS map_cities ON stg_rides.pickup_city_id = map_cities.city_id 
        
    
        
            LEFT JOIN uber.bronze.map_cancellation_reasons AS map_cancellation_reasons ON stg_rides.cancellation_reason_id = map_cancellation_reasons.cancellation_reason_id 